...
public class RegistroHospede extends JFrame {
	...
	public RegistroHospede(Long id) {
		...
		btnsalvar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (txtDataN.getDate() != null && txtNome.getText() != "" && txtSobrenome.getText() != ""
						&& txtTelefone.getText() != "") {
					salvaHospede();
	...
	public void salvaHospede() {
		String nascimento = ((JTextField) txtDataN.getDateEditor().getUiComponent()).getText();

		Hospede novoHospede = new Hospede(txtNome.getText(), txtSobrenome.getText(), java.sql.Date.valueOf(nascimento),
				txtNacionalidade.getSelectedItem().toString(), txtTelefone.getText(),
				Long.parseLong(txtNreserva.getText()));

		controller.inserir(novoHospede);
